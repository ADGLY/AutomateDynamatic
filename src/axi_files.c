...
auto_error_t advance_in_file(FILE *file,
    ...
    regmatch_t match[1];
    CHECK_CALL(find_pattern(pattern, *offset, 1, match), "find_pattern failed !")
...
auto_error_t write_top_file(project_t *project, axi_ip_t *axi_ip) {
    ...
    CHECK_CALL_DO(
            advance_in_file(new_top_file, &top_file_off, pattern),
    ...
    CHECK_CALL_DO(advance_in_file(new_top_file, &top_file_off,
    ...
    CHECK_CALL_DO(
            advance_in_file(new_top_file, &top_file_off,
    ...
    CHECK_CALL_DO(
            advance_in_file(new_top_file, &top_file_off, "port [(]"),
            "advance in file failed !", fclose(new_top_file);)
...
auto_error_t write_axi_file(project_t *project, axi_ip_t *axi_ip) {
    ...
    CHECK_CALL_DO(
            advance_in_file(new_axi_file, &axi_file_off,
                            "C_S_AXI_DATA_WIDTH\\s+\\:\\s+integer\\s+:=\\s+32;"),
    ...
    CHECK_CALL_DO(advance_in_file(new_axi_file, &axi_file_off,
                                  "-- Users to add ports here"),
                  "advance in file failed !", fclose(new_axi_file);)
    ...
    CHECK_CALL_DO(
            advance_in_file(new_axi_file, &axi_file_off, pattern),
            "advance in file failed !", fclose(new_axi_file);)
