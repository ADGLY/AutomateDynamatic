...
auto_error_t get_entity_name(hdl_info_t *hdl_info) {
    regmatch_t match[2];
    CHECK_CALL(find_pattern("entity[[:space:]]*(\\w+)[[:space:]]*is", hdl_info->source, 2, match),
               "find_pattern failed !")
...
auto_error_t get_ports_list(hdl_info_t *hdl_info, list_type_t type) {
    ...
    while ((size_t) (source_off - hdl_info->source) <= end_of_port &&
           find_set_pattern(source_off, 3, matches, 1) != ERR_REGEX) {
        ...
        if (count == alloc_size) {
            if (type == ARRAYS) {
                CHECK_CALL_DO(grow_array((void **) (&list), &alloc_size, sizeof(hdl_array_t)), "grow_array failed !",
                              free(list);)
            } else if (type == PARAMS) {
                CHECK_CALL_DO(grow_array((void **) (&list), &alloc_size, sizeof(hdl_in_param_t)), "grow_array failed !",
                              free(list);)
            }
        }
...
auto_error_t get_end_out_width(hdl_info_t *hdl_info) {
    regmatch_t match[2];
    CHECK_CALL(
            find_pattern("end_out[[:space:]]*:[[:space:]]*out[[:space:]]*std_logic_vector[[:space:]]*\\(([[:digit:]]+)",
                         hdl_info->source, 2, match), "find_pattern failed !")
